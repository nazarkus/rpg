...
local function attackVehicle(td)
    ...
    if off[td.Type] then
        pos = pos + Vector3.new(0, off[td.Type], 0)
    end
...
local function updPStat()
    local c = 0
    for _ in pairs(selP) do c = c + 1 end
    local w = 0
    for _ in pairs(wlP) do w = w + 1 end
    if c == 0 and w == 0 then
...
pSelAll.MouseButton1Click:Connect(function()
    ...
    for p, el in pairs(plrEl) do
        if selP[p] then
            ...
            if rs then
                TS:Create(rs, TweenInfo.new(0.1), {Color = C.SelBrd, Transparency = 0.3}):Play()
            end
...
pClear.MouseButton1Click:Connect(function()
    ...
    for p, el in pairs(plrEl) do
        if not wlP[p] then
            ...
            local rs = el:FindFirstChildOfClass("UIStroke")
            if rs then
                TS:Create(rs, TweenInfo.new(0.1), {Color = C.Border, Transparency = 0.7}):Play()
            end
            for _, child in pairs(el:GetChildren()) do
                if child:IsA("Frame") and child.Size == UDim2.new(0, 16, 0, 16) then
...
pToggle.MouseButton1Click:Connect(function()
    ...
    if pSpamOn then
        local n = 0
        for _ in pairs(selP) do n = n + 1 end
        if n == 0 then
...
local function updVStat()
    local n = 0
    for _ in pairs(selV) do n = n + 1 end
    if n == 0 then
...
local function makeVehRow(tgt)
    ...
    local nl = Instance.new("TextLabel")
    nl.Text = tgt.Name
    nl.Size = UDim2.new(0, 140, 0, 14)
    nl.Position = UDim2.new(0, 70, 0, 5)
    nl.BackgroundTransparency = 1
    nl.TextColor3 = C.Text
    nl.Font = Enum.Font.GothamBold
    nl.TextSize = 11
    ...
    ol.Name = "VehInfo"
    ol.Size = UDim2.new(0, 220, 0, 11)
    ol.Position = UDim2.new(0, 70, 0, 22)
...
plr.CharacterAdded:Connect(function(nc)
    ...
    if rpgReady and rpgDot then
        rpgDot.BackgroundColor3 = C.Success
    end
end)
