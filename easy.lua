...
local C = {
    ...
    CardH    = Color3.fromRGB(24, 24, 40),
    Input    = Color3.fromRGB(14, 14, 24),
    Border   = Color3.fromRGB(30, 30, 48),
    BorderL  = Color3.fromRGB(40, 40, 60),
...
-- ============================================================
-- CHECK IF PLAYER HAS RPG
-- ============================================================
local function playerHasRPG(player)
    if not player then return false end
    pcall(function()
        if player.Character and player.Character:FindFirstChild("RPG") then return true end
        local bp = player:FindFirstChild("Backpack")
        if bp and bp:FindFirstChild("RPG") then return true end
    end)
    -- Direct check
...
-- ============================================================
-- SHIELD BREAKER — THROTTLED (не спамит слишком быстро)
-- ============================================================
...
local function breakShield(player)
    ...
    task.spawn(function()
        ...
        -- Замедленный shield break — 15 итераций с паузой 0.15с
        for attempt = 1, 15 do
            ...
            -- Бить по 3 части за раз максимум
            local hitCount = 0
            ...
            task.wait(0.15) -- Пауза между залпами
        end
    end)
end
...
rpgDot.Position = UDim2.new(0, 20, 0, 54)
...
onlineLbl.Size = UDim2.new(0.5, -4, 0, 26)
...
searchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    task.defer(function()
        local scroll = pContent:FindFirstChild("PlayerScroll")
        if scroll then
...
pScroll.Size = UDim2.new(1, 0, 0, 210)
pScroll.Position = UDim2.new(0, 0, 0, 70)
pScroll.BackgroundColor3 = C.Panel
pScroll.BackgroundTransparency = 0.3
pScroll.ScrollBarThickness = 2
pScroll.ScrollBarImageColor3 = C.Accent
pScroll.ScrollBarImageTransparency = 0.5
...
local pLay = Instance.new("UIListLayout"); pLay.Padding = UDim.new(0, 2); pLay.Parent = pScroll
pad(pScroll, 4, 4, 4, 4)

-- Status
local pStatLbl = Instance.new("TextLabel")
pStatLbl.Size = UDim2.new(1, 0, 0, 26)
pStatLbl.Position = UDim2.new(0, 0, 0, 286)
...
local function createPlrEl(player)
    ...
    row.Parent = pScroll
    cr(row, 6)
    local rowStroke = sk(row, C.Border, 1, 0.7)
    ...
    cm.TextColor3 = C.White; cm.Font = Enum.Font.GothamBold; cm.TextSize = 11
    cm.ZIndex = 7; cm.Parent = cb

    -- Avatar
    local avFrame = Instance.new("Frame")
    avFrame.Size = UDim2.new(0, 26, 0, 26)
    avFrame.Position = UDim2.new(0, 28, 0.5, -13)
    avFrame.BackgroundColor3 = C.Input
    avFrame.ZIndex = 6; avFrame.Parent = row; cr(avFrame, 13)

    local av = Instance.new("ImageLabel")
    av.Size = UDim2.new(1, -2, 1, -2)
    av.Position = UDim2.new(0, 1, 0, 1)
    av.BackgroundTransparency = 1; av.ZIndex = 7
    av.Parent = avFrame; cr(av, 12)
    ...
    nl.Size = UDim2.new(0, 90, 0, 14)
    nl.Position = UDim2.new(0, 60, 0, 5)
    ...
    -- Sub info: @name · base · dist · RPG
    local infoLbl = Instance.new("TextLabel")
    infoLbl.Name = "InfoLbl"
    infoLbl.Size = UDim2.new(0, 180, 0, 11)
    ...
    local function updateInfo()
        ...
        local parts = {}
        table.insert(parts, '<font color="#46465F">@'..player.Name..'</font>')
        ...
        if dist then
            table.insert(parts, '<font color="#46465F">'..dist..'m</font>')
        end
    ...
    shBtn.Text = "⚡"
    shBtn.Size = UDim2.new(0, 20, 0, 20)
    shBtn.Position = UDim2.new(1, -48, 0.5, -10)
    ...
    wlBtn.Name = "WL"
    wlBtn.Text = "🛡"
    wlBtn.Size = UDim2.new(0, 20, 0, 20)
    wlBtn.Position = UDim2.new(1, -24, 0.5, -10)
    ...
    wlBtn.Parent = row; cr(wlBtn, 4)

    local clickBtn = Instance.new("TextButton")
    clickBtn.Size = UDim2.new(1, -54, 1, 0)
    ...
    local function vis()
        ...
        if isWL then
            TS:Create(row, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(18, 25, 45)}):Play()
            TS:Create(rowStroke, TweenInfo.new(0.15), {Color = C.WL, Transparency = 0.3}):Play()
        ...
        elseif isSel then
            ...
            TS:Create(rowStroke, TweenInfo.new(0.15), {Color = C.SelBrd, Transparency = 0.3}):Play()
            ...
            TS:Create(rowStroke, TweenInfo.new(0.15), {Color = C.Border, Transparency = 0.7}):Play()
            TS:Create(cb, TweenInfo.new(0.15), {BackgroundColor3 = C.Input}):Play()
            cbStroke.Color = C.Border; cm.Text = ""
            TS:Create(wlBtn, TweenInfo.new(0.15), {BackgroundColor3 = C.Input}):Play()
            wlBtn.TextColor3 = C.TextMute; nl.TextColor3 = C.Text
    ...
    row:SetAttribute("_update", "yes")
    plrEl[player] = row
    
    -- Store updateInfo for later refresh
...
pBtnFrame.Size = UDim2.new(1, 0, 0, 28)
pBtnFrame.Position = UDim2.new(0, 0, 0, 318)
...
pSelAll.MouseButton1Click:Connect(function()
    ...
    for p, el in pairs(plrEl) do
        if selP[p] then
            ...
            if rs then TS:Create(rs, TweenInfo.new(0.1), {Color = C.SelBrd, Transparency = 0.3}):Play() end
...
pClear.MouseButton1Click:Connect(function()
    ...
    for p, el in pairs(plrEl) do
        if not wlP[p] then
            ...
            local rs = el:FindFirstChildOfClass("UIStroke")
            if rs then TS:Create(rs, TweenInfo.new(0.1), {Color = C.Border, Transparency = 0.7}):Play() end
            for _, child in pairs(el:GetChildren()) do
                if child:IsA("Frame") and child.Size == UDim2.new(0, 16, 0, 16) then
...
pToggle.Position = UDim2.new(0, 0, 0, 352)
...
local pInfoLbl = Instance.new("TextLabel")
pInfoLbl.Text = "Shield detection  ·  ⚡ break shield  ·  🛡 whitelist"
pInfoLbl.Size = UDim2.new(1, 0, 0, 16)
pInfoLbl.Position = UDim2.new(0, 0, 0, 400)
pInfoLbl.BackgroundTransparency = 1
pInfoLbl.TextColor3 = C.TextMute
pInfoLbl.Font = Enum.Font.Gotham
pInfoLbl.TextSize = 9
pInfoLbl.ZIndex = 4
pInfoLbl.Parent = pContent
...
vCntLbl.Size = UDim2.new(0.5, -4, 0, 26)
...
vScroll.Size = UDim2.new(1, 0, 0, 260)
vScroll.Position = UDim2.new(0, 0, 0, 38)
vScroll.BackgroundColor3 = C.Panel
vScroll.BackgroundTransparency = 0.3
vScroll.ScrollBarThickness = 2
vScroll.ScrollBarImageColor3 = C.Accent
vScroll.ScrollBarImageTransparency = 0.5
...
local vLay = Instance.new("UIListLayout"); vLay.Padding = UDim.new(0, 2); vLay.Parent = vScroll
pad(vScroll, 4, 4, 4, 4)

local vStatLbl = Instance.new("TextLabel")
vStatLbl.Size = UDim2.new(1, 0, 0, 26)
vStatLbl.Position = UDim2.new(0, 0, 0, 304)
...
local function makeVehRow(tgt)
    ...
    badge.Position = UDim2.new(0, 28, 0.5, -9)
    badge.BackgroundColor3 = vCol[tgt.Type] or C.Card
    badge.BackgroundTransparency = 0.6
    badge.ZIndex = 6; badge.Parent = row; cr(badge, 4)

    local bTxt = Instance.new("TextLabel")
    bTxt.Text = vShort[tgt.Type] or "??"
    bTxt.Size = UDim2.new(1,0,1,0); bTxt.BackgroundTransparency = 1
    bTxt.TextColor3 = C.Text; bTxt.TextSize = 8; bTxt.Font = Enum.Font.GothamBlack
    bTxt.ZIndex = 7; bTxt.Parent = badge

    -- Name
    local nl = Instance.new("TextLabel")
    nl.Text = tgt.Name
    nl.Size = UDim2.new(0, 140, 0, 14); nl.Position = UDim2.new(0, 70, 0, 5)
    nl.BackgroundTransparency = 1; nl.TextColor3 = C.Text
    nl.Font = Enum.Font.GothamBold; nl.TextSize = 11
    nl.TextXAlignment = Enum.TextXAlignment.Left
    nl.TextTruncate = Enum.TextTruncate.AtEnd; nl.ZIndex = 7; nl.Parent = row

    -- Info line: owner · base · distance
    local ol = Instance.new("TextLabel")
    ol.Name = "VehInfo"
    ol.Size = UDim2.new(0, 220, 0, 11); ol.Position = UDim2.new(0, 70, 0, 22)
    ol.BackgroundTransparency = 1; ol.Font = Enum.Font.Gotham; ol.TextSize = 9
    ...
    local function updateVehInfo()
        ...
        if tgt.DisplayName and tgt.DisplayName ~= "" then
            ...
            parts[#parts+1] = '<font color="#46465A">No owner</font>'
        end
        local dist = getDistanceToVeh(tgt.HRP)
        if dist then
            parts[#parts+1] = '<font color="#46465A">'..dist..'m</font>'
        end
...
-- Vehicle buttons — only Select All / Clear All
local vBtnFrame = Instance.new("Frame")
vBtnFrame.Size = UDim2.new(1, 0, 0, 28)
vBtnFrame.Position = UDim2.new(0, 0, 0, 336)
...
vToggle.Position = UDim2.new(0, 0, 0, 370)
...
local vInfoLbl = Instance.new("TextLabel")
vInfoLbl.Text = "Auto-refresh 3s  ·  Owner + Base + Distance"
vInfoLbl.Size = UDim2.new(1, 0, 0, 16)
vInfoLbl.Position = UDim2.new(0, 0, 0, 418)
vInfoLbl.BackgroundTransparency = 1
vInfoLbl.TextColor3 = C.TextMute
vInfoLbl.Font = Enum.Font.Gotham
vInfoLbl.TextSize = 9
vInfoLbl.ZIndex = 4
vInfoLbl.Parent = vContent
...
-- Update loop: shields, info, distances
task.spawn(function()
    while true do
        for p, el in pairs(plrEl) do
